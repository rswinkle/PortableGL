...
void gouraud_ads_vp(float* vs_output, pgl_vec4* vertex_attribs, Shader_Builtins* builtins, void* uniforms)
{
	...
	vec3 eye_normal = u->normal_mat * vec3(vert_attribs[ATTR_NORMAL]);
	...
	*(vec4*)&builtins->gl_Position = u->mvp_mat * (vert_attribs[ATTR_VERTEX] + vec4(vec3(vert_attribs[ATTR_INSTANCE]), 0));
...
void phong_ads_vp(float* vs_output, pgl_vec4* vertex_attribs, Shader_Builtins* builtins, void* uniforms)
{
	...
	*(vec3*)vs_output = u->normal_mat * vec3(vert_attribs[ATTR_NORMAL]);
