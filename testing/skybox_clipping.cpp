...
GLFrame camera_frame(true, vec3(0, 0, 3));
...
int main(int argc, char** argv)
{
	...
	make_box(box_verts, box_tris, box_tex, 2, 2, 2, GL_FALSE, ivec3(1.0f), vec3(-1.0f));
	...
	rsw::make_perspective_matrix(projection, DEG_TO_RAD(45), float(WIDTH)/float(HEIGHT), 0.1f, 100.0f);
	...
	while (1) {
		...
		//cout << the_uniforms.mvp_mat << endl;
...
void skybox_vs(float* vs_output, pgl_vec4* vertex_attribs, Shader_Builtins* builtins, void* uniforms)
{
	...
	vec4 pos = the_uniforms->mvp_mat * v_attrs[0];
