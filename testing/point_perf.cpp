...
int main(int argc, char** argv)
{
	...
	vector<vec3> points;
#define NUM_PER_SET 10000

	for (int i=0; i < NUM_PER_SET; ++i) {
		points.push_back(vec3(rsw::rand_float(-1, 0), rsw::rand_float(-1, 1), -1));
	}
	for (int i=0; i < NUM_PER_SET; ++i) {
		points.push_back(vec3(rsw::rand_float(-1, 1), rsw::rand_float(-1, 0), 1));
	}
	...
	while (!quit) {
		...
		glDrawArrays(GL_POINTS, 0, NUM_PER_SET);
		the_uniforms.v_color = Blue;
		glDrawArrays(GL_POINTS, NUM_PER_SET, NUM_PER_SET);
		the_uniforms.v_color = Red;

		glDrawArrays(GL_POINTS, 2*NUM_PER_SET, 1);
		the_uniforms.v_color = Blue;
		glDrawArrays(GL_POINTS, 2*NUM_PER_SET+1, 1);
		the_uniforms.v_color = Red;
...
void setup_context()
{
	...
	SDL_Window* window = SDL_CreateWindow("swrenderer", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, WIDTH, HEIGHT, SDL_WINDOW_SHOWN | SDL_WINDOW_RESIZABLE);
	if (!window) {
