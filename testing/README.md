...
in the existing tests. FBO / RTT / MRT regression cases live in `test_fbo.cpp` (names
`fbo_*`); design notes for those images are in `notes/fbo_tests.md`. RGB565
`fbo_*_RGB565` goldens are change-detectors for the 16-bit window path—not proof of
16-bit offscreen color attachments (those still use RGBA8 or float texture storage).

Mipmap / LOD / Grad cases are in the PNG suite (`test_mipmap.cpp`: `mipmap_unit`,
`mipmap_auto_vis`, `mipmap_lod_bands`, `mipmap_grad_vis`); design notes in
`notes/mipmap_tests.md`. Optional standalone C smoke tests remain for CORE/UNSAFE
builds and quick loops.
