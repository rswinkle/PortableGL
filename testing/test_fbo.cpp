...
static void fbo_mrt_white_fs(float* fs_input, Shader_Builtins* builtins, void* uniforms)
{
	PGL_UNUSED(fs_input);
	PGL_UNUSED(uniforms);
	builtins->gl_FragData[0] = make_v4(1.f, 1.f, 1.f, 1.f);
	builtins->gl_FragData[1] = make_v4(1.f, 1.f, 1.f, 1.f);
}

// ColorMaski(0): only R (and A). Buffer 1 unmasked. Draw white over black.
void test_fbo_colormask_i(int argc, char** argv, void* data)
{
	PGL_UNUSED(argc);
	PGL_UNUSED(argv);
	PGL_UNUSED(data);
	...
	GLuint prog = pglCreateProgram(fbo_identity_vs, fbo_mrt_white_fs, 0, NULL, GL_FALSE);
	glUseProgram(prog);
	GLuint vbo;
	glGenBuffers(1, &vbo);
	glBindBuffer(GL_ARRAY_BUFFER, vbo);
	...
	vec4 c0 = texelFetch2D(tex0, tw / 2, th / 2, 0);
	vec4 c1 = texelFetch2D(tex1, tw / 2, th / 2, 0);
	PGL_EXPECT(c0.x > c0.y && c0.x > c0.z, "buf0 mask keeps G/B black");
	PGL_EXPECT(c1.y > c0.y && c1.z > c0.z, "buf1 unmasked writes G/B");

	glColorMask(GL_TRUE, GL_TRUE, GL_TRUE, GL_TRUE);
	GLboolean wm[4];
	glGetBooleanv(GL_COLOR_WRITEMASK, wm);
	PGL_EXPECT(wm[0] && wm[1] && wm[2] && wm[3], "ColorMask restores all");
